    ...
    >
      <div className="flex-1">
        <div className="font-semibold text-base md:text-lg flex items-center gap-2">
          ...
          {status === SESSION_STATUS.CURRENT && <span className="ml-2 text-xs bg-braini-blue/20 text-braini-blue-dark px-2 py-0.5 rounded">Actual</span>}
          {status === SESSION_STATUS.COMPLETED && <span className="ml-2 text-xs bg-braini-turquoise/20 text-braini-turquoise-dark px-2 py-0.5 rounded">Completada</span>}
        </div>
        <div className="text-braini-blue font-black text-lg md:text-xl mt-1" style={{ fontWeight: 900 }}>{titulo}</div>
