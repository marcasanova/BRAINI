...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: invite, error: inviteError } = await supabaseAdmin
      .rpc("get_director_invite_by_token", { p_token: token })
      .single();

    if (inviteError) {
      console.error("get_director_invite_by_token error:", inviteError.message);
    ...
    if (completeError) {
      console.error("complete_director_invite error:", completeError.message);
    ...
    return new Response(
      JSON.stringify({
        ok: true,
        director_id: completeData?.director_id ?? userId,
    ...
    console.error("complete-director-invite error:", msg);
